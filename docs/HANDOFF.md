...
| `8b13266` + `846023b` | Merge slice 42 (Negotiation Coach AI) + C-9 (design foundation). **42:** on-demand "Coach me" in a deal's bids tab → `computeNegotiationStats` over that partner's prior decided bids + an AI sentence via the slice-32 seam (feature `negotiation-coach`), deterministic `simulatedCoaching` when keyless. **Owner-only is the security core** — `getNegotiationCoaching` verifies `deal.orgId === session.orgId` BEFORE any history read or AI call; review empirically confirmed bidder-org / unrelated-third-org / bogus-dealId / demo-third-org all return Forbidden with ZERO seam calls, ZERO reader calls, ZERO Sentry. **BUY/SELL inverts the math** (`deals.kind` is from the OWNER's view: SELL→want highest, BUY→want lowest) and each *historical* close is scored by THAT deal's own kind — a regression test proves the naive per-current-kind version scores backwards. Read-only, no migration/writes/deps; on-demand by design so it never bills the gateway per dashboard render. Review fixes: cross-tenant org-name oracle closed (label falls back to a local `Org {id}`, never `resolveOrgLabel` on an unvalidated caller-supplied id), empty-reduce 500 guarded, negative decide-days clamped, and the phrase "win rate" dropped from card + prompt (denominator excludes deals lost to competing bidders — it would over-claim on advice the owner acts on). **C-9:** see the ROADMAP row — design foundation shipped separately and in parallel (disjoint files); the component sweep is C-10, now unblocked. | core |
