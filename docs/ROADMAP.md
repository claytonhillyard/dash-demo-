...
| C-8 | Neon transaction driver (neon-http → neon-serverless) + `FOR UPDATE` on the overpay guard | core | claimed 2026-07-31 | **this tab** | The real-deploy blocker: neon-http throws on transaction() so recordPayment/35b confirms fail closed on live Postgres. Needs a real Neon deploy smoke-test to verify — hence its own row, not bundled into C-7. |
