...
| 26 | WinJewel CSV import wizard (W2) | core (WinJewel preset seam) | shipped: `4e36ed1` | this-tab | /customers/import wizard. Idempotent UPSERT on (org_id, external_ref); in-house RFC-4180 parser; byte-accurate 5MB cap. 126 tests. Works in the desktop build — real AIYA roster loadable today. W3 (invoices, slice 27) is next in the arc. |
| 27 | Invoice schema + create/edit form (W3) | core | shipped: `0be14ae` | this-tab | Migration 0020. bill_to frozen at issue; void-not-delete; editable numbering (slice 30's door). Review fixed a codebase-wide dead constraint-error path (drizzle cause-unwrap) + an int4 total overflow. 118 tests (1556 green). W4 (PDF+email, slice 28) next. |
| 28 | Invoice PDF + email send (W4) | core | shipped: `09d7859` | this-tab | Migration 0021 (sent_at/sent_to). pdf-lib 1.17.1 model/painter split; sendInvoice (issued-only, simulated-no-stamp); GET /invoices/[id]/pdf. Review caught 2 Criticals: non-WinAnsi chars (CJK) crashed pdf-lib → model-level sanitize; route helper export broke next build. ~120 new tests (1625+ green). W5 (payments, slice 29) next. |
| 29 | Payments + balance tracking (W5) | core | shipped: `500fd77` | this-tab | Migration 0022 (payments). Balance derived (total − SUM), paid-state a badge not a status. Transactional overpay guard (pglite-serialized; FOR UPDATE + Neon driver chipped). Review fixed a reproduced concurrent double-delete/double-audit race via DELETE…RETURNING. ~85 tests (1689 green pre-fix run). W6 (history import, slice 30) next. |
...
| 31 | Document vault (contracts, NDAs) | core | shipped: `2d05a3c` | this-tab | Migration 0024 (documents, org-scoped + optional customer link). Upload PDF/image (magic-byte validated by actual bytes), list, stream-download via a route handler + a new `BlobStore.get` (v10 has no signed URLs). `/documents` page + nav. ~80 tests (2303 green). Review: merge, magic-byte unbypassable + downloads attachment/no-store. Greenfield core, avoided the deals/bids domain. |
...
| 35a | AI Command Layer — read-only palette | core | shipped: `db808f8` | this-tab | /command: NL → whitelisted 8-command registry (AI routes question→intent+params ONLY; NO business data in prompts — structural, hallucinated numbers impossible) → org-scoped readers → inline results. Keyless via deterministic rules matcher. ~100 tests (2160 green). Review: merge, no Critical/Major. |
| 35b | AI Command Layer — confirmed write actions | core | shipped: `9f324c8` | this-tab | 3 write commands (record payment, send invoice, add note) behind a preview→Confirm contract. **The AI is never in the execute path** (verified empirically across 12 review probes) — it routes + previews read-only; Confirm delegates to the existing guarded actions (all session/org/demo/overpay/audit intact). No migration/deps/new write path. ~120 tests (2225 green). Void/destructive writes deliberately deferred. |
...
| 42 | Negotiation Coach AI (in-thread) | core | shipped: `8b13266` | this-tab | On-demand "Coach me" in the bids tab → grounded stats over prior decided bids with THIS partner + AI sentence (deterministic fallback keyless). **Owner-only** (a bidder gets Forbidden with zero AI calls / zero history reads — review-verified against 4 rejection shapes); BUY/SELL inversion correct in all 4 places, each historical close scored by ITS OWN deal kind. Read-only, no migration/deps. ~73 tests (2378 green). Review closed a cross-tenant org-name oracle + dropped over-claiming "win rate" wording. First slice in the deals domain since the other tab went dead. |
...
| C-7 | Hardening pass: `__setTestDb` prod guard + dashboard per-panel degradation + Tailwind `text` token | core | shipped: `3db3a74` | this-tab | 3 chips resolved: `__setTestDb` no-ops outside test env across **14** action files (not 7 — recount) with a proven-real regression test; `safePanel` degrades a failing panel fetch (Sentry-captured) instead of 500ing the dashboard, incl. the deal-independent todaysBids fetch (review catch); `text: hsl(var(--text))` defined so ~387 `text-text/*` classes finally mute. No migration/deps. 2229 tests green. |
| C-9 | Design foundation — "Assay Office" (tokens, elevation, focus, motion, grain, display face) | core | shipped: `846023b` | this-tab | Foundation only (globals.css + tailwind.config + layout) so it could run parallel to slice 42. Fixed REAL defects the audit found: no `color-scheme: dark` (~150 native controls rendering light-mode), 43% of muted text below WCAG AA, ZERO focus rings, 92/96 hover states with no transition, bg-vs-surface at 1.05:1, ZERO tabular figures. Cormorant → Newsreader; Inter deliberately kept for dense data. 2305 green. Audit: `docs/superpowers/specs/2026-07-31-ui-redesign-audit.md`. |
| C-10 | Design sweep — components (opacity ladder → solid tokens, tabular money, admin shell, panel dialects, icons, states) | core | claimed 2026-07-31 | **this tab** | The other half of the redesign. Blocked only by slice 42 (both touch DealBidsTab). Includes: migrate ~438 `text-text/NN` call sites to `text-muted`/`text-dim` (the actual AA fix), `.nums` on every money cell, add `src/app/(admin)/layout.tsx` (all 23 admin pages currently render with NO Shell — no sidebar/topbar), unify the 3 competing panel dialects, an icon system (1 `<svg>` app-wide today, rest emoji), wire loading/empty/error states (`Panel`'s are dead code; no `loading.tsx`/`error.tsx` anywhere). ⚠ Respect the audit's test-coupling map. |
| C-8 | Neon transaction driver (neon-http → neon-serverless) + `FOR UPDATE` on the overpay guard | core | shipped: `867b490` | this-tab | Real-deploy unlock: neon-serverless (WebSocket, via `ws` on Node 20) gives real transactions so recordPayment/35b confirms stop failing closed on live Postgres; `FOR UPDATE` row lock closes the slice-29 F2 overpay race. Pglite path proven unchanged (2230 green + tsc + build). **⚠ Neon runtime path is DEPLOY-verified, not CI-verified — run the `docs/DEPLOY-NEON.md` smoke-test on first real deploy (PENDING USER).** |
