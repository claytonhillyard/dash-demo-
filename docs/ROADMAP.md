...
| 35 | AI Command Layer (NL → action) | core | claimed 2026-07-23 (35a scope) | **this tab** | 35a = read-only palette: NL → whitelisted command registry (AI routes question→intent+params ONLY — no data in prompts, structural) → org-scoped readers → inline results; keyless via deterministic rules matcher. 35b (confirmed write actions) stays open. |
