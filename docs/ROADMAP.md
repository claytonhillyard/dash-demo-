...
| 35b | AI Command Layer — confirmed write actions | core | proposed | open | Extends 35a's registry with WRITE commands (record payment, send draft, add note…) behind an explicit confirm step; audited. The 35a router/palette/help substrate is reused; each write command declares a preview + confirmation contract. |
