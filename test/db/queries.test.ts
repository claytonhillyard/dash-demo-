...
describe("queries against pglite", () => {
  ...
  it("profit + margin for the current month", async () => {
  ...
  it("operating margin: returns null when revenue is zero (divide-by-zero guard)", async () => {
    await db.insert(profitMonths).values({ year: Y, month: M, amountCents: 25_00 });
    expect(await getCurrentOperatingMarginPct(db, Y, M)).toBeNull();
  });
  ...
  it("trailing 12 months: revenue + profit + clients-added by acquired_on", async () => {
    ...
    expect(series[0]).toMatchObject({ revenueCents: 0, profitCents: 0, clientsAdded: 0 });
  });
